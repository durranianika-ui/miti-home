...
export const VAT_RATE = numberFromEnv("NEXT_PUBLIC_VAT_RATE", 0.05);
export const PRICES_INCLUDE_VAT = booleanFromEnv("NEXT_PUBLIC_PRICES_INCLUDE_VAT", true);

// Shipping & fees (AED). Placeholder commercial terms — confirm before launch.
export const FREE_SHIPPING_THRESHOLD = numberFromEnv("NEXT_PUBLIC_FREE_SHIPPING_THRESHOLD_AED", 300);
export const FREE_SHIPPING_THRESHOLD_DISPLAY = `AED ${FREE_SHIPPING_THRESHOLD.toLocaleString("en-AE")}`;
export const SHIPPING_FEE = numberFromEnv("NEXT_PUBLIC_SHIPPING_FEE_AED", 25);
export const COD_ENABLED = booleanFromEnv("NEXT_PUBLIC_COD_ENABLED", true);
export const COD_FEE = numberFromEnv("NEXT_PUBLIC_COD_FEE_AED", 10);
/** Orders above this total must be prepaid (courier cash-handling limit). */
export const COD_MAX_ORDER_TOTAL = numberFromEnv("NEXT_PUBLIC_COD_MAX_ORDER_AED", 2500);
...
function emiratesFromEnv(name: string): readonly UaeEmirate[] {
...
export const SHIPPING_EMIRATES = emiratesFromEnv("NEXT_PUBLIC_SHIPPING_EMIRATES");
/** Emirates where the courier collects cash on delivery. */
export const COD_ALLOWED_EMIRATES = emiratesFromEnv("NEXT_PUBLIC_COD_EMIRATES");

// Bargain AI (optional; off unless explicitly enabled)
export const BARGAIN_AI_ENABLED = booleanFromEnv("NEXT_PUBLIC_FEATURE_BARGAIN_AI", false);
...
export const RETURN_WINDOW_DAYS = numberFromEnv("NEXT_PUBLIC_RETURN_WINDOW_DAYS", 7);
export const EXCHANGE_WINDOW_DAYS = numberFromEnv("NEXT_PUBLIC_EXCHANGE_WINDOW_DAYS", 7);
